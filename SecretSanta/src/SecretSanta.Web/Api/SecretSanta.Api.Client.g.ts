...
export interface IGroupsClient {
    ...
    addAssignment(id: number): Promise<void>;
}
...
export class GroupsClient implements IGroupsClient {
    ...
    addAssignment(id: number , cancelToken?: CancelToken | undefined): Promise<void> {
        ...
        return this.instance.request(options_).catch((_error: any) => {
            ...
            return this.processAddAssignment(_response);
        });
    }

    protected processAddAssignment(response: AxiosResponse): Promise<void> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (let k in response.headers) {
